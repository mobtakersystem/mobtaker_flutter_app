...
class CheckOtpPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      ...
      smartAuth.getSmsWithRetrieverApi().then(
        (value) {
          if (value.hasData) {
            final code = value.requireData.code;
            if (code != null) {
              _formKey.currentState?.fields['otp']?.didChange(code);
              ref.read(loginFlowProvider.notifier).phoneCheckOtp(code);
            }
          }
        },
      );
